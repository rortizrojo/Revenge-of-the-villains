...
public class Mario extends Personaje implements IColisionable {
    ...
    public void acciones() throws SlickException {
        ...
        switch (estado){
            ...
            case 5: //HUYENDO              
                if (jugador_a_izq){//jugador_a_izq=true -> jugador a izq. de Mario
                    botonDerecha = true; 
                }
                else{ 
                    botonIzquierda = true;                  
    ...
    public void actualizarEstado(float jugadorX, float jugadorY) {  
        ...
        if (vida>40 && distancia<=700 && distancia > 680) 
            estado = 1; //ESTADO EN GUARDIA = 1
        else if (vida>40 && distancia<=680 && distancia > 400) 
            estado = 2; //ESTADO ACERCANDOSE = 2
        else if (vida>40 && distancia<=680) 
            estado = 3; //ESTADO ATACANDO = 3
        else if (vida<=40 && vida>5 && distancia<=680) 
